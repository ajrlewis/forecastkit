...
frequency = "D"  # "infer" for automatic alignment, but specific offsets are most reliable, 'D' is daily
...
if initial_training:
    ...
    ensemble = [
        ...
        "dist",
        "simple",
    ]  # , "mosaic", "mosaic-window", 'mlensemble'
elif evolve:
    gens = 500
...
if graph:
    with plt.style.context("bmh"):
        ...
        prediction.plot_grid(
            ...
            title="Worst Performing Forecasts",
            cols=worst,
        )
        plt.show()
        ...
        prediction.plot_grid(
            model.df_wide_numeric,
            start_date=start_date,
            title="Best Performing Forecasts",
            cols=best,
        )
        plt.show()
        ...
        if model.best_model_name == "Cassandra":
            ...
            plt.show()
            prediction.model.plot_trend(series=None, start_date=start_date)
            plt.show()
        ...
        if model.best_model_ensemble == 2:
            ...
            if str(model_parameters["model_name"]).lower() in [
                "mosaic",
                "mosaic-window",
            ]:
